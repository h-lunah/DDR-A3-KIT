...
local songSteps = song:GetAllTrails()
...
for pn in ivalues(GAMESTATE:GetEnabledPlayers()) do
	...
	--SOUND:PlayOnce(THEME:GetPathS("ScreenSelectMusic","Select Level "..RSL));
		for i,steps in ipairs(songSteps) do
			if steps:GetDifficulty() == GAMESTATE:GetPreferredDifficulty(pn) and steps:GetStepsType() == GAMESTATE:GetCurrentStyle():GetStepsType() then
				selection[pn] = i
				GAMESTATE:SetCurrentTrail(pn,steps)
				foundDiff = true
			end;
		end;
	end;
	if not foundDiff then
		GAMESTATE:SetCurrentTrail(pn,songSteps[1])
	end;
end;
	...
	for i,steps in ipairs(songSteps) do
		...
		f[i] = Def.ActorFrame{
			InitCommand=function(s)
				s:xy(math.abs(i-center)*getXSpacing(player),(i-center)*Y_SPACING)
				if steps:GetStepsType() ~= GAMESTATE:GetCurrentStyle():GetStepsType() then
					s:visible(false)
				end
			end,
...
local function DiffInputHandler(event)
	...
	if (button == "MenuUp" or button == "MenuLeft") and selection[pn] > 1 and GAMESTATE:IsPlayerEnabled(pn) and keyset[pn] ~= 1 then
		if songSteps[selection[pn] - 1]:GetStepsType() == GAMESTATE:GetCurrentStyle():GetStepsType() then
			SOUND:PlayOnce(THEME:GetPathS("","ScreenSelectMusic difficulty harder"));
			selection[pn] = selection[pn] - 1
			GAMESTATE:SetCurrentTrail(pn,songSteps[selection[pn]])
