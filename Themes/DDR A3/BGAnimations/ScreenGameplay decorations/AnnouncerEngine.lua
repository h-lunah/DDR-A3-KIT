...
local globalCooldown = 2 * #GAMESTATE:GetEnabledPlayers()-- Global cooldown period in seconds
...
local currentSong = nil -- Current song
...
t[#t+1] = Def.ActorFrame{
    JudgmentMessageCommand=function(self, params)
        ...
        if playerCombos[pn] > 1000 and playerCombos[pn] % 100 == 0 and playerLastComboMilestonePlayed[pn] ~= playerCombos[pn] and GAMESTATE:GetPlayerState(pn):GetPlayerController() ~= "PlayerController_Autoplay" then
            SOUND:PlayAnnouncer("combo overflow ac")
            playerLastComboMilestonePlayed[pn] = playerCombos[pn]
            lastSoundPlayedTime = currentTime
        return end

        if playerCombos[pn] > 0 and playerCombos[pn] % 100 == 0 and playerLastComboMilestonePlayed[pn] ~= playerCombos[pn] and GAMESTATE:GetPlayerState(pn):GetPlayerController() ~= "PlayerController_Autoplay" then
        ...
        if playerCombos[pn] > 0 or playerMissCombos[pn] > 0 then
            if playerCombos[pn] == 0 and playerMissCombos[pn] > 0 then
                playerCombos[pn] = playerMissCombos[pn]
            end
